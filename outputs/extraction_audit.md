...
- **Composite-dedup's weakness exposed**: with phase and primary_outcome removed from the composite key (Phase 2 relaxation), the key reduces to (sponsor first-word, drug INN, N ± 5%). Two distinct trials that happen to enrol similar N — here DELIVER N=6,263 appearing in both a correct supplement review and a mislabelled one — merge incorrectly. A stronger key would require primary_outcome substring overlap after text normalisation, but that demands the outcome-text extractor to be reliable first (Task 16 completion).
- **Ship-ready Turner-analysis path** is still blocked: the current 5-row CSV has 2 exact-match rows, 2 noisy, and 1 partial-noisy (DELIVER N exact but HR is the CV-death-alone secondary, not the primary composite). Cohort-level inflation analysis needs ≥20 clean rows.
- **Next concrete unlocks** (not attempted in this session): (a) FDA-site manual-verification per sNDA supplement, (b) a second scrape step that reads the Drugs@FDA approval-label history page to map supplement→indication, (c) CV-death-composite vs CV-death-alone disambiguation in the HR extractor.
